...
function translateSession(urlSession) {
  let name = ''
  if (urlSession[0] == 'P') {
    return 'Poster Session ' + urlSession[1]
  } else {
    if (urlSession[0] == 'O') {
      return 'Oral Session ' + urlSession[1]
    }
  }
  return urlSession
}
...
const updateSession = () => {
    ...
    if (urlSession) {
        ...
        d3.select('#session_name').text(translateSession(urlSession));
